    ...
    computed: {
        getIframeUrl() {
            if (this.$router.currentRoute.value.query.resume) {
                this.iframeSrc = `${PLAYER_URL}player.php?movie=` + this.$route.params.playerId + "&about=1&streams=1&videos=1&channels=1&matches=1&movies=1&tv_shows=1&apps=0&chat=0&donation=1&epg=0&packages=1&resume={this.$router.currentRoute.value.query.resume}";
            } else {
                this.iframeSrc = `${PLAYER_URL}player.php?movie=` + this.id + "&about=1&streams=1&videos=1&channels=1&matches=1&movies=1&tv_shows=1&apps=0&chat=0&donation=1&epg=0&packages=1";
            }
