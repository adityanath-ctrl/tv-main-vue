...
import { getSubscriptionEvents, getGuestToken, isTokenExpired } from '@/utils/siberAPI'; // Import token helpers
...
const props = defineProps({
  providerId: {
    type: Number as PropType<number>, // Keep prop as number if that's what parent passes
    required: true
  }
});
...
onMounted(async () => {
  ...
  if (!token || isTokenExpired(token)) {
    ...
    // console.warn(`ProviderSubscriptionEvents: Token invalid/missing for provider ${props.providerId}, attempting guest token.`);
    try {
      ...
      if (guestTokenResponse.data?.access_token) {
        ...
        console.error(`ProviderSubscriptionEvents: Failed to get guest token for provider ${props.providerId}.`);
        rawPackages.value = [];
        return;
      }
    } catch (error) {
      console.error(`ProviderSubscriptionEvents: Error fetching guest token for provider ${props.providerId}:`, error);
      rawPackages.value = [];
      return;
    }
  }
  ...
  if (!token) {
    console.error(`ProviderSubscriptionEvents: No valid token to fetch subscriptions for provider ${props.providerId}.`);
    rawPackages.value = [];
    return;
  }
  ...
  try {
    ...
    // -----------------------------------------------------------
    // console.log(`ProviderSubscriptionEvents: Fetching subscriptions for provider ${props.providerId} with params:`, apiParams);
    const res = await getSubscriptionEvents(apiParams);
    // console.log(`ProviderSubscriptionEvents: API Response for provider ${props.providerId}:`, JSON.parse(JSON.stringify(res)));

    if (res.data?.response?.packages && Array.isArray(res.data.response.packages)) {
      rawPackages.value = res.data.response.packages;
    } else {
      // console.warn(`ProviderSubscriptionEvents: No packages found or invalid format for provider ${props.providerId}. Response:`, res);
      rawPackages.value = [];
    }
  } catch (error) {
    console.error(`ProviderSubscriptionEvents: Error fetching subscriptions for provider ${props.providerId}:`, error);
    rawPackages.value = [];
  }
});

// Flatten into individual entries for display
const liveEvents = computed((): PackageDisplayItem[] => // Changed name to reflect it's packages
...
function goToPackage(id: number | string) { // id can be number or string
  // Assuming event packages also go to a generic package detail page
  router.push(`/svod/package/${id}`); // Adjust path if event packages have a different route
}
