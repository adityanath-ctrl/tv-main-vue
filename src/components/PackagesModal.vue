<template>
    <v-layout row justify-center>
        ...
        <v-dialog v-model="dialog" max-width="1200" hide-overlay="false">
            <v-card class="rounded-lg" style="background-color: rgb(17 17 17 / 100%);">
                ...
                <v-tabs v-model="tab" align-tabs="center" v-if="!isShowDetailPackage" class="ml-10">
                    <v-tab v-for="(item, index) in PackageCategoryItems" :value="item.packageId"
                        class=" pa-4 mx-4 rounded-lg" style="border: 0px;">
                        {{ item.packageTitle }}
                    </v-tab>
                </v-tabs>

                <v-responsive class="overflow-y-auto" max-height="645">
                    <v-window v-model="tab">
                        <v-window-item v-for="(item, index) in PackageCategoryItems" :key="item.packageId"
                            :value="item.packageId">
                            <v-container fluid>
                                <v-row class="pl-2 pr-4" v-if="isShowDetailPackage">
                                    <v-col cols="5" md="5">
                                        <v-img :src="selectedPackageItem.packageDetailImg" aspect-ratio="2"
                                            style="cursor: pointer;"></v-img>
                                        <v-card-title class="text-h5 pa-5 d-flex justify-space-between">
                                            <div style="color: green;">Price: {{ selectedPackageItem.packagePrice }} (usd) /
                                                month</div>
                                        </v-card-title>
                                    </v-col>
                                    <v-col cols="7" md="7">
                                        ...
                                        <div class="text-h6 d-flex justify-space-between align-center mb-5"
                                            style="width: 40vw;">
                                            {{ selectedPackageItem.packageTitle }}
                                            <p class="text-caption text--disabled">{{ selectedPackageItem.packagePrice }}
                                            </p>
                                        </div>
                                        ...
                                        <div class="d-flex justify-start align-center mb-5">
                                            <v-btn class="d-flex justify-flex-end align-center pr-4 ml-2"
                                ...
                                <v-row>
                                    <v-card-title v-if="isShowDetailPackage"
                                        class="text-h5 pl-10 pt-5 pb-1 d-flex justify-space-between">
                                        <div>{{ selectedPackageItem.packageCategory }} included in Packages</div>
                                    </v-card-title>
                                </v-row>
                            </v-container>
                            ...
                            <v-container fluid class="pa-7">
                                <v-row v-for="(detailItem, detailIndex) in PackageDetailItems" :key="detailIndex"
                                    class="pa-2" style="border-top: 1px solid #ddd;cursor: pointer;"
                                    @click="onSelectPackage(detailItem)">
                                    <v-col cols="3" md="3">
                                        <v-img :src="detailItem.packageDetailImg" aspect-ratio="2"></v-img>
                                    </v-col>
                                    <v-col cols="9" md="9">
