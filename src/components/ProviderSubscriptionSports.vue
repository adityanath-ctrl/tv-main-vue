<!-- File: src/components/ProviderSubscriptionSports.vue -->
<template>
...
import { getSubscriptionSports, getGuestToken, isTokenExpired } from '@/utils/siberAPI'; // Import token helpers
...
onMounted(async () => {
  ...
  if (!token || isTokenExpired(token)) {
    ...
    // console.warn(`ProviderSubscriptionSports: Token invalid/missing for provider ${props.providerId}, attempting guest token.`);
    try {
      ...
      if (guestTokenResponse.data?.access_token) {
        ...
        console.error(`ProviderSubscriptionSports: Failed to get guest token for provider ${props.providerId}.`);
        rawPackages.value = [];
        return;
      }
    } catch (error) {
      console.error(`ProviderSubscriptionSports: Error fetching guest token for provider ${props.providerId}:`, error);
      rawPackages.value = [];
      return;
    }
  }
  ...
  if (!token) {
    console.error(`ProviderSubscriptionSports: No valid token to fetch subscriptions for provider ${props.providerId}.`);
    rawPackages.value = [];
    return;
  }
  ...
  try {
    ...
    // -----------------------------------------------------------
    // console.log(`ProviderSubscriptionSports: Fetching subscriptions for provider ${props.providerId} with params:`, apiParams);
    const res = await getSubscriptionSports(apiParams);
    // console.log(`ProviderSubscriptionSports: API Response for provider ${props.providerId}:`, JSON.parse(JSON.stringify(res)));

    if (res.data?.response?.packages && Array.isArray(res.data.response.packages)) {
      rawPackages.value = res.data.response.packages;
    } else {
      // console.warn(`ProviderSubscriptionSports: No packages found or invalid format for provider ${props.providerId}. Response:`, res);
      rawPackages.value = [];
    }
  } catch (error) {
    console.error(`ProviderSubscriptionSports: Error fetching subscriptions for provider ${props.providerId}:`, error);
    rawPackages.value = [];
  }
});
...
function goToPackage(id: number | string) { // id can be number or string
  // Assuming sports packages also go to a generic package detail page
  router.push(`/svod/package/${id}`); // Adjust path if sports packages have a different route
}
