...
export const EventsStore = {
    ...
    actions: {
        setLiveEventsByCategoryId(context : ActionContext < any, any >, playload : any): void {
            ...
            getLiveEventsByCategoryId(apiParams).then(res => {
                let events = {
        ...
        getEventsByContentProviderId(context : ActionContext < any, any >, payload : any): void {
            ...
            if (tokenExpired(token) || auth.auth.tokenMode == "guest") {
                getGuestToken().then((value : any) => {
                    ...
                    getEventsByContentProviderId(apiParams).then((res : any) => {
                        // var filtered_events;
                        // if (payload.category == 'video') {
                        //     filtered_events = res.data.response.events.filter(item => item.live_event_playback_status == "uploaded" || item.live_event_playback_status == "pending_upload");
                        // } else {
                        //     filtered_events = res.data.response.events.filter(item => item.live_event_playback_status == "uploaded" || item.live_event_playback_status == "upload_pending");
                        // }
        ...
        getFilmsByContentProviderId(context : ActionContext < any, any >, payload : any): void {
            ...
            if (tokenExpired(token) || auth.auth.tokenMode == "guest") {
                getGuestToken().then((value : any) => {
                    ...
                    getFilmsByContentProviderId(apiParams).then((res : any) => {
        ...
        getSeriesByContentProviderId(context : ActionContext < any, any >, payload : any): void {
            ...
            if (tokenExpired(token) || auth.auth.tokenMode == "guest") {
                getGuestToken().then((value : any) => {
                    ...
                    getSeriesByContentProviderId(apiParams).then((res : any) => {
